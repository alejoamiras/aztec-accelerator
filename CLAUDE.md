...
- **CI**: GitHub Actions (PR gates: `accelerator.yml`, `sdk.yml`, `app.yml`, `actionlint.yml`; deploy: `deploy-landing.yml`; publish (workflow_dispatch only): `publish-testnet.yml`; reusable: `_e2e.yml`, `_e2e-app.yml`, `_e2e-webdriver.yml`, `_e2e-updater-windows.yml` (real accelerator-v1.0.7 N−1 + 4-point fixture preflight), `_publish-sdk.yml`, `_aztec-update.yml`; manual smoke: `smoke-updater-windows.yml` (workflow_dispatch, secretless ephemeral both-ends build + L8 barrier scenario proving the update-window marker mid-NSIS); @aztec bump tooling (workflow_dispatch only, `merge_mode: none` leaves the PR open — F-008): `aztec-stable.yml`; release: `release-accelerator.yml`)
- **Testing**: 12 WebDriver E2E tests (macOS + Linux + Windows) via `tauri-plugin-webdriver` + WebdriverIO, 35 Playwright UI mock tests, ~115 Rust unit tests, ~96 TS unit tests, plus `#[ignore]`d real-OS integration suites run per-OS in CI (`trust_*`, `autostart_heal`). WebDriver tests run as PR gate and pre-release gate.

...
- **Accelerator** (`/packages/accelerator`): Tauri desktop app — system tray, multi-version bb cache, optional HTTPS (Safari), crash recovery, site authorization (MetaMask-style origin approval with `url::Url`-based canonicalization at server ingress + curated verified-sites registry rendering friendly name + ✓ in the popup; see `VERIFIED_SITES.md`), Settings window, speed control, signed auto-update (Ed25519 via tauri-plugin-updater).

...
- **Testing**: 12 WebDriver E2E tests (macOS + Linux + Windows) via `tauri-plugin-webdriver` + WebdriverIO, 35 Playwright UI mock tests, ~115 Rust unit tests, ~104 TS unit tests, plus `#[ignore]`d real-OS integration suites run per-OS in CI (`trust_*`, `autostart_heal`). WebDriver tests run as PR gate and pre-release gate.

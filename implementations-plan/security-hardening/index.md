...
| C5 | `sechard/infra-deploy-authz` | F-005 | deep | C0 | IN-PROGRESS | — | GATE 1 blueprint (deep). Scope narrowed: **nightlies dropped** (branch unused per user 2026-07-14). Live pipelines only: deploy-landing (`landing/`), release (`landing/releases/latest.json` = F-004-critical), publish-testnet (`playground/`). Decisions: per-pipeline prefix-scoped OIDC roles (release-feed isolated); OIDC sub → `main` only (drop nightlies + chore/aztec-*); main protection = 3 checks + linear + no-force + convo-resolution, keep 0 approvals (solo); CI = tofu fmt+validate (human applies). clusters/C5-* |

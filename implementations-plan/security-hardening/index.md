...
| C5 | `sechard/infra-deploy-authz` | F-005 | deep | C0 | GATE1-DONE·AWAIT-APPROVAL | — | **GATE 1 COMPLETE**: 3 plans → double audit (Fable *conditional approve* 8 findings + Codex *reject* 7 findings, ALL folded) → final Codex verdict **APPROVE** (round 5). Design: 3 per-pipeline OIDC roles bound via the `workflow` NAME claim (D1 resolved a Fable↔Codex contradiction on job_workflow_ref); release-feed isolated to exact `latest.json`; landing `--exclude releases/*` + IAM Deny (fixes latent feed-delete bug); disable unused publish-nightlies; main ruleset +linear/no-force/deletion/thread-res + required Actionlint check, keep 0 approvals; multipart lifecycle; S3 versioning ask. Commit+validate only (human applies). **Awaiting user approval before GATE 2.** clusters/C5-* (CONSOLIDATED, eli5, audits, 3 legs) |

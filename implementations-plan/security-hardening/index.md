...
| C4 | `sechard/updater-rollback` | F-004 | deep | C0 | IN-PROGRESS | #387 | GATE 1 deep (3 plans + double-audit REJECT folded: B1 capability bypass, B2 flock, B3 outer-feed binding). GATE 2 done in 8 commits: Layer A verifier (0) + Layer B floor (2) + capability denial (1a) + VerifiedUpdate newtype/verify-gate/signed-size cap (1b) + floor-commit tracker + fs2 flock + TOCTOU (1c) + update-manifest tool (3a) + least-priv sign-update-feed job (3b) + PR-gate round-trip E2E (4). GATE 4 local: clippy -D + fmt + actionlint clean, 161 core + 25 src-tauri tests, full throwaway-key round-trip proven (splice/size tamper → OuterMismatch). GATE 3 codex + GATE 5 CI in flight. clusters/C4-*, lessons/phase-C4.md (2 codex consults died mid-run → resolved empirically + per blueprint) |

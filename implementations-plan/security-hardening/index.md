...
| C4 | `sechard/updater-rollback` | F-004 | deep | C0 | IN-PROGRESS | — | **GATE 1 DONE** (84bc486): 3 plans→consolidated→double-audit REJECT→folded. Design: base64-verbatim signed manifest envelope (verify-after-check, VerifiedUpdate newtype, deny updater:default capability) + cross-process flock version floor. GATE 2 (impl) next: update_manifest.rs + updater_state.rs + updater.rs + build.rs + sign-update-feed CI job + PR-safe E2E. clusters/C4-* |

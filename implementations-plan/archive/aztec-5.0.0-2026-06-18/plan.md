...
**Closed 2026-06-19.** deep→**SDK-only**, ✅ **shipped 2026-06-18** (PR #363 merged; SDK `5.0.0-rc.1` on npm `testnet`; canonical salt=0 SponsoredFPC deployed+funded on v5 `0x2613…7880`; FPC bridge scripts + deploy decoupling #364; playground live on v5; WASM CRS stale-cache fix #365) — `@aztec/* 4.3.1 → 5.0.0-rc.1` Aztec-5.0 HARD FORK. 3 parallel plans (main+opus+codex; Fable down→opus sub) → 2 audits + final codex (all conditional-approve, folded). **Release shape: SDK npm publish only** — the deployed accelerator fetches+runs 5.0 bb at runtime (`prove.rs`/`downloader.rs`), so no app re-release IFF the bb `prove` CLI/msgpack/proof interface is unchanged (verified empirically by P4 = the accelerator-backed local-5.0 E2E, now a standing gate via `sdk.yml build_accelerator:true`; RED→Appendix C contingency rebuild). Scope: no Noir contracts → aztec.nr changelog N/A; real surface = SDK proving imports + playground getTxReceipt(mined-reverted)/v5 host. Decisions: min-age documented override; SDK on `testnet` tag not `latest`; no new live-testnet CI gate (manual v5 smoke); no /harden. v5 canonical salt=0 FPC NOT published on testnet (live smoke needs the testnet FPC salt). False alarm corrected mid-audit: is_valid_version DOES accept `5.0.0-rc.1` (version_policy.rs:195 + test)

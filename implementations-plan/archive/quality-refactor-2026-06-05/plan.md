...
- **Q6 UpdateCoordinator** — ❌ **CUT (owner, 2026-06-06, resolves Ask F).** On reading `updater.rs`: 130 lines, 3 cohesive + heavily-documented functions, one-sentence responsibility, NO in-file test net (proof only via the Windows updater-smoke gate), entirely on the safety-critical disarm→install→rearm brick-path. The Extract-Class wrap adds reshuffle-risk for ~zero maintainability gain — it violates the "don't split a one-sentence module" principle. Dropped from the rc batch.
- **Q7 SafariSupportManager** — preserve the safety-critical lifecycle order (Q7 consolidates the missing recovery path). *(Extract Class.)* [SAFETY-CRITICAL — rc dry-run.]
...
### Phase 8 — SDK type break (Q5 + Q12) ✓ — Q5·1 #probeHealth done #306 (Q5·2 PhaseReporter = churn, skipped); Q12 AcceleratorStatus union ✓ MERGED #315 (+MIGRATION.md); phase-event union = ❌ CUT (marginal value: a 2nd phase type tying durationMs to "proved" across ~25 sites, fighting the playground's string-based AnimationPhase model; same negative-ROI logic as the Q6/Q13 cuts — owner-overridable + reversible pre-publish, request it anytime and it ships as a follow-up); playground needs NO migration (zero AcceleratorStatus refs, verified) [BREAKING — dev line; stable publish joint]
...
- **Ask F — RESOLVED (owner, 2026-06-06): CUT Q6.** Confirmed after reading `updater.rs` (130 lines, 3 already-clean documented fns, no in-file test net, safety-critical brick-path). Q6 dropped from Phase 7 + the rc batch; the rc now validates Q1/Q2/Q4/Q7 only. Q13 (copy-bb matrix, cosmetic) decision deferred to Phase 9.

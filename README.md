...
> **Chrome Local Network Access.** Public sites need Chrome's permission to reach the loopback
> accelerator. An explicit denial is surfaced as `permission-blocked` so an app can show Site
> settings guidance and Retry; an unresolved/dismissed prompt can still look `offline`. The SDK's
> loopback annotation does not bypass permission, and HTTPS is subject to the same address-space gate.

...
test("upgrader prefill: autostart reflects current state", async ({ page }) => {
  await page.addInitScript(() => {
    (window as any).__TAURI_MOCK__.setHandler("get_onboarding_state", () => ({
      platform: "macos",
      https_default: true, // pre-checked for everyone (A9)
      autostart_enabled: false,
      auto_update: false,
      trust_status: { stores: [] },
    }));
...
test("Start invokes complete_onboarding with the toggle states and closes on success", async ({
  ...
  await page.goto("/onboarding.html");
  await page.locator("#opt-auto-update").uncheck();

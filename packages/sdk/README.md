...
> The SDK's version mirrors the Aztec line it targets — SDK `X.Y.Z` is built against `@aztec/*` `X.Y.Z`. A bare install resolves npm `latest`, the build promoted as current-stable; the `testnet` dist-tag always points at the newest published build (`npm install @alejoamiras/aztec-accelerator@testnet`). The two are usually the same version, and differ while a newer line is being validated or if `latest` has been rolled back.
